...
[CustomizeSearchPath("Functionals/Mathematicals/Vector Scale Node")]
public class VectorScaleNode : BaseRuntimeNode {
    ...
    private void Evaluate(NodeEvaluationInfo info, [OutputLayoutIndex(0)] out MVector output) {
        output = info.Container.EvaluateInput<ITractatoriConvertible>(LeftHandSide).ToMVector() * info.Container.EvaluateInput<ITractatoriConvertible>(LeftHandSide).ToMVector().X;
    }
}
